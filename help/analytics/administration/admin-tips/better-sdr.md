...
_Revolucione sua estratégia de dados e capacite sua equipe a criar um documento sólido de Referência de design de solução (SDR). Eliminar lacunas de medição e promover uma cultura de dados colaborativa por meio de metodologias passo a passo._

É finalmente hora. Você junta um SDR sólido. Um SDR é o guia usado para implementar suas métricas e dimensões. Você definiu como eles são chamados quando são disparados e seus desenvolvedores adoram. Você passou por todo o processo de implantação, escreveu critérios de aceitação, passou pelos seus limites, testou e está pronto! Sua instância de [!DNL Adobe Analytics] As equipes de marketing e de produtos devem comemorar à medida que analisam os dados, obtêm novas revelações sobre os clientes e encontram todas as áreas de sucesso e, bem, áreas de menos sucesso. Mas não está ouvindo os elogios que esperava.
...
_Por que os membros da sua equipe não estão adotando isso?_

Vou começar deixando você um pouco fora do gancho. Há _sempre_ será uma revisão. Se o site ou aplicativo for complexo o suficiente para precisar de uma solução de análise corporativa, você perderá algo. Mas neste caso, você não perdeu o suficiente para explicar as falhas de medição que estou descrevendo.
...
Quero apresentar um método que meus colegas e eu desenvolvemos para criar um SDR melhor com menos intervalos e para fazer com que os usuários finais investissem (e mesmo ocasionalmente entusiasmados) sobre sua nova instância de [!DNL Adobe Analytics]. Vamos analisar como e por que você deve considerar esse método.

## O How

_Saiba mais sobre a conferência de medição. Use um mapa de funil para visualizar cada etapa do plano. Crie painéis de modelos para revisar como um grupo. Crie um dicionário de dados para usuários._
