...
##### 2.1.1.1. Workspace/Definir Tela Inicial Personalizada ([!DNL Adobe Analytics])
...
Aqui, você encontra os relatórios de &quot;Bot&quot;. O tráfego de bots é excluído de todos os seus relatórios padrão. No entanto, há dois relatórios que fornecem o insight sobre o que está acontecendo e quais bots estão chegando ao seu site. Isso é especialmente bom se você configurar Regras de bot personalizadas para excluir bots de spam conhecidos que acessam frequentemente seu site. Você pode colocar insight no que esses bots estão fazendo sem que seus relatórios principais sejam inundados por esse tráfego. Os relatórios de bot estão indisponíveis no Workspace no momento (mas os novos recursos de relatório em breve também permitirão que os usuários obtenham essas informações lá).
...
O Workspace do [!DNL Adobe] (Seção 2.1.2.1) usa a API para criar os relatórios e, se você habilitar o modo de depuração no Workspace, ele mostrará as chamadas de API exatas usadas. Essa é uma maneira rápida de criar suas chamadas de API. Ao usar o Workspace para criar e validar os dados que deseja obter, você pode usar essas chamadas de API para obter os dados para seu próprio processamento.
